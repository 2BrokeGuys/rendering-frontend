...
export default function Page() {
  ...
  return (
    <section id="pricing" className="py-20 md:py-32 border-b bg-muted/50">
            <div className="container">
              <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
                <h2 className="text-3xl md:text-4xl font-bold">Choose the perfect plan for your needs</h2>
                <p className="text-muted-foreground text-lg">
                  From individual artists to large studios, we have pricing options that scale with your requirements.
                </p>
                <div className="flex items-center justify-center gap-2 pt-4">
                  <span className={!isAnnual ? "font-medium" : "text-muted-foreground"}>Monthly</span>
                  <Switch checked={isAnnual} onCheckedChange={setIsAnnual} id="pricing-toggle" className="SwitchRoot" />
                  <span className={isAnnual ? "font-medium" : "text-muted-foreground"}>Annual (20% off)</span>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-8">
                {[
                  ...
                  <div
                    key={i}
                    className={`relative rounded-lg border ${plan.popular ? "border-primary shadow-lg" : ""} bg-background hover:drop-shadow-xl`}
                  >
                    {plan.popular && (
                      <div className="absolute top-0 right-0 transform translate-x-2 -translate-y-2">
                        <Badge className="px-3 py-2 m-2 bg-primary text-slate-50 ">Most Popular</Badge>
                      </div>
                    )}
